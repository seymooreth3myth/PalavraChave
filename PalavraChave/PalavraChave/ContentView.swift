...
//  PalavraChave
//
//  Created by Fábio Carlos de Souza on 04/02/23.
//
...
struct ContentView: View {
    ...
    func addNewWord() {
        ...
        guard isPossible(word: answer) else {
            wordError(title: "Não é possível usar essa palavra", message: "Você não pode usar essa palavra de '\(rootWord)'")
            return
        }
        
        guard isReal(word: answer) else {
            wordError(title: "Palavra não reconhecida", message: "Não invente uma palavra, você sabe!")
            return
        }
